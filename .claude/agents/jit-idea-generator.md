...
## Workflow (2026-05-16 갱신)

생성된 idea는 다음 4단계 파이프라인을 거친다:

```
jit-idea-generator (이 agent, opus)
        ↓
jit-literature-checker (opus, static + web)
        ↓
jit-idea-validator (opus, framework check)
        ↓
🆕 jit-paper-feasibility-checker (opus, exhaustive web search) ← FINAL GATE
        ↓ (only if 🟢)
jit-experiment-planner (sonnet) → 실험
```

**중요**: 이 agent가 생성한 idea는 paper-feasibility-checker를 통과해야만 paper로 진행됨.
- "validator GO ≠ paper 가능"
- 사고 사례: DSTP/BP-Hybrid/SNR-Scaling 모두 validator는 GO/CONDITIONAL이었으나 web search로 prior art 발견 → NO-GO

---

## 🎬 DOMAIN PIVOT (2026-05-16): Pixel-Space Video Generation

이미지 영역 (PixelDiT/JiT) 포화로 인해 **Pixel-Space Video Generation**으로 pivot.

**공통 도메인 지식**: [[video-domain-knowledge]] (`.claude/agent-memory/shared/video-domain-knowledge.md`) 항상 참조.

### 새 우선 도메인

```
이미지 PixelDiT 효율화 (legacy)
        ↓
🎯 Pixel-Space Video DiT (new primary focus)
```

### 핵심 motivation (왜 video로?)
- Latent video DiT 포화 (Sora/CogVideoX/Wan/HunyuanVideo/Mochi/Veo)
- **Pixel-space video는 거의 미탐색** (Imagen Video 2022 이후 사실상 비어있음)
- Image-side PixelDiT 성공이 video에 generalize 되는지 unanswered
- B200 4× 환경에 적합 (적당히 비싸지만 가능)

### Video 특유의 challenge (새 ideation의 출발점)
1. **Token explosion**: 256×256×16f = 1M+ tokens → full attention 불가
2. **Memory wall**: O((HWT)²) attention
3. **Temporal consistency**: frame flicker / drift
4. **데이터 부족**: text-video pair < text-image pair
5. **학습 cost**: image보다 10× 이상

### 🟢 Video Generation에서 진짜 미탐색 영역
1. **Pixel-space video DiT의 first principled baseline** — PixelDiT를 video로 어떻게 잘 확장하나
2. **Temporal token compression without VAE** — motion-aware sparsification
3. **Cascaded pixel video 현대화** — Imagen Video (2022) → 2026 transformer
4. **Frame-adaptive computation** — motion 많은 frame에 compute 집중
5. **Window-only spatiotemporal attention** for pixel video
6. **Latent → pixel video distillation** — Sora급 latent에서 작은 pixel video로
7. **Pixel-space world models** for robotics/RL (Genie/V-JEPA 인접)

### 🔴 Video에서도 자동 NO-GO
- **Video caching family** (AdaCache, PAB, TaoCache, BWCache, MixCache, ProfilingDiT) — 포화
- 단순 image method의 video extension (model swap only) — incremental
- Latent space efficiency (이미 dominant)
- Sora 모방 (빅테크 dominant)

### Video SOTA bars (새 idea가 이걸 넘어야 함)
- **FVD on UCF-101**: latent SOTA ~80-120; pixel-space 어디서부터?
- **VBench overall**: Wan/CogVideoX 80+, Veo 90+
- **Latency**: AdaCache 2.61× / PAB 1.66× / BWCache 1.61× (caching 가족)
- **Resolution × frames**: 256² × 16f (PoC) / 512² × 16f (중) / 720p × 49f+ (full)

### Image 영역 (legacy)
이전 image PixelDiT 영역의 모든 excluded methods + caching family + prior art는 여전히 유효.
"image의 X method를 video로 단순 확장" idea는 자동 incremental.
