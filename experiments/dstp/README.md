...
| F1 | FID-10K @ 100 step (paper-grade) | B200 | 🔄 진행중 | (생성 중) |
| F2 | DeepCache analog vs DSTP | B200 | ✅ | 비슷 (1.36-1.59×) |
| F3 | JiT-B/16 일반화 | B200 | ✅ | monolithic→이득 적음 |
...
## 결과 6: JiT-B/16 일반화 시도

| Config | median ms | speedup | 비고 |
|---|---|---|---|
| Baseline | 48.17 | 1.00× | JiT-B/16, 50 step |
| K3_tbased | 325.75 | 0.15× | torch.compile cache miss로 추정 |
| K3_periodic | 42.50 | 1.13× | speedup 미미 |

**핵심 발견**: JiT는 monolithic 구조 (12 또는 32 blocks)라 PixelDiT의 patch/pixel 분할 구조 같은 caching 이점이 없다.
DSTP의 효과는 단순 step caching이 아니라 **architecture-aware caching**(patch=cache, pixel=refine)에서 옴을 시사.
...
3. **IS 단독 신뢰 불가** — K3_periodic은 IS drop 0.27인데 시각적으로 심각한 손상
4. **20 step PoC: 1.53×, 100 step canonical: 1.27× (paired)**
5. **B200 timing 노이즈 매우 큼** — 외부 사용자 간섭. paired/격리 환경에서만 신뢰 가능
6. **DeepCache analog와 비슷한 성능** (1.36-1.59×) — DSTP는 더 단순한 special case
7. **JiT (monolithic)에는 효과 작음** — 본 방법은 PixelDiT의 dual-level architecture에 특화
...
1. ✅ FID-50K → FID-10K 진행 중 (overnight)
2. ⬜ JiT-H/16 (32 blocks) 부분 caching: blocks[0:24] cache + blocks[24:32] always run
3. ⬜ DiP 직접 비교 (retrained vs training-free trade-off)
4. ⬜ 다양한 batch size에서 speedup 안정성
5. ⬜ 시각 평가 user study
